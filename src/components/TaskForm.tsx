        ...
        <form onSubmit={submit} className="bg-slate-900 border border-slate-800 rounded-xl p-4 space-y-3">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                ...
                <div className="md:col-span-2">
                    ...
                    placeholder="例：○○社向け、期限は金曜"rows={3}/>
                </div>
                <div>
                    <label className="text-sm text-slate-300">ステータス</label>
                    <select className="mt-1 w-full px-3 py-2 rounded-lg bg-slate-950 border border-slate-800" value={status} onChange={(e) => setStatus(e.target.value as TaskStatus)}>
                        {statusOptions.map((o) => (
                ...
                <div>
                    ...
                    <select className="mt-1 w-full px-3 py-2 rounded-lg bg-slate-950 border border-slate-800" value={priority} onChange={(e) => setPriority(e.target.value as TaskPriority)}>
            ...
            <button className="px-4 py-2 rounded-lg bg-emerald-600 hover:bg-emerald-500 disabled:opacity-60" disabled={loading || !title}>
                {loading ? "追加中..." : "タスク追加"}
            </button>
        </form>
    );
